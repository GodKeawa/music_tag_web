    ...
    <div style="display: flex;flex-wrap: wrap;">
        <div class="file-section">
            <div style="width: 95%;margin-top: 20px;margin-left: 10px;">
                ...
                <div style="margin-top: 10px;display: flex;align-items: center;">
                    ...
                    <bk-checkbox style="margin-left: 10px; min-width: 120px;" v-model="filterFailed" @change="handleSearchFile">仅显示未成功</bk-checkbox>
        ...
        <bk-dialog v-model="exampleSetting1.primary.visible"
            ...
            <div style="margin-top: 15px;">
                <bk-checkbox v-model="skipProcessed">跳过已成功项 (状态变灰的文件不再反复挂削)</bk-checkbox>
            </div>
        </bk-dialog>
    ...
    export default {
        ...
        methods: {
            ...
            handleSearchFile() {
                ...
                this.checkedData = []
                this.checkedIds = []
                this.$api.Task.fileList({'file_path': this.filePath, sorted_fields: this.sortedField, filter_failed: this.filterFailed}).then((res) => {
                    if (res.result) {
                        if (this.filterFailed && res.data && res.data.length > 0) {
                            const rootNode = res.data[0]
                            const resData = []
                            for (const each of rootNode.children) {
                                if (each.state !== 'success') {
                                    resData.push(each)
                                }
                            }
                            rootNode.children = resData
                        }
                ...
                this.$bkInfo({
                    ...
                    confirmFn: () => {
                        try {
                            ...
                            this.musicInfoManual['skip_processed'] = this.skipProcessed
