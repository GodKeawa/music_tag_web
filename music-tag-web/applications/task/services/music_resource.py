...
class MusicResource:
    ...
    def fetch_id3_by_id(self, song_id):
        try:
            if hasattr(self.resource, 'fetch_id3_by_id'):
                return self.resource.fetch_id3_by_id(song_id)
            return []
        except Exception as e:
            print("音乐平台获取详情失败", e)
            return []
...
class NetEaseMusicClient:
    ...
    def fetch_id3_by_id(self, song_id):
        data = send({"url": self.BASE_URL + "api/song/detail", "params": {"ids": f"[{song_id}]"}}, "linuxapi").POST("")
        try:
            songs = data.json().get("songs", [])
        except Exception as e:
            print("网易云音乐详情获取失败", e, data.text)
            songs = []
        for song in songs:
            artists = song.get("ar", [])
            album = song.get("al", {})
            if artists:
                ...
                artist = ""
                artist_id = ""
            # publishTime is at song level in detail API
            year = song.get("publishTime", "")
            if year:
                year = timestamp_to_dt(year / 1000, "%Y")
            song["artist"] = artist
