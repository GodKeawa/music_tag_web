...
def update_music_info(music_id3_info, is_raw_thumbnail=False):
    for each in music_id3_info:
        ...
        full_path_clean = each["file_full_path"].replace("//", "/")
        parent_path = os.path.dirname(full_path_clean)
        filename = os.path.basename(full_path_clean)
        Task.objects.update_or_create(full_path=full_path_clean, defaults={
            "state": "success",
