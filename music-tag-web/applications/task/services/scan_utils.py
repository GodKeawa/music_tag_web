...
class MusicInfo:
    ...
    @property
    def comment(self):
        val = str(self.file["comment"].value or "")
        try:
            if self.file.mfile and hasattr(self.file.mfile, 'tags') and self.file.mfile.tags:
                if isinstance(self.file.mfile.tags, dict) or hasattr(self.file.mfile.tags, 'items'):
                    items = self.file.mfile.tags.items()
                else:
                    items = self.file.mfile.tags
                for k, v in items:
