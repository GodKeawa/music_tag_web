...
class TaskViewSets(GenericViewSet):
    ...
    def file_list(self, request, *args, **kwargs):
        ...
        file_path = validate_data['file_path'].replace('//', '/')
        ...
        filter_failed = validate_data.get('filter_failed', False)
        
        def has_unprocessed_files(folder_path):
            folder_prefix = folder_path.replace('//', '/') + "/"
            success_files = set(Task.objects.filter(full_path__startswith=folder_prefix, state="success").values_list("full_path", flat=True))
            for root, dirs, files in os.walk(folder_path):
                root_forward = root.replace("\\", "/").replace("//", "/")
                for file in files:
                    file_type = file.split(".")[-1]
                    if file_type not in ALLOW_TYPE:
                        continue
                    file_full_path = f"{root_forward}/{file}".replace("//", "/")
                    if file_full_path not in success_files:
                        return True
            return False
        ...
        for index, entry in enumerate(file_data, 1):
            ...
            if entry.get("is_dir", None):
                if filter_failed:
                    if not has_unprocessed_files(entry.get("path")):
                        continue
    ...
    def batch_auto_update_id3(self, request, *args, **kwargs):
        ...
        skip_processed = music_info.get("skip_processed", False)
        timestamp = str(int(time.time() * 1000))
        bulk_set = []
        for each in select_data:
            name = each.get("name")
            song_name = ".".join(name.split(".")[:-1])
            full_item_path = f"{full_path}/{name}".replace("//", "/")
            
            if skip_processed and each.get("icon") != "icon-folder":
                task_record = Task.objects.filter(full_path=full_item_path).first()
                if task_record and task_record.state == "success":
                    print(f"[{name}] 已成功挂削，根据跳过选项已跳过")
                    continue
        ...
        batch_auto_tag_task(timestamp, source_list, select_mode, skip_processed)
        return self.success_response()
