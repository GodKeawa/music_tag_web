...
def batch_auto_tag_task(batch, source_list, select_mode, skip_processed=False):
    """
    自动刮削任务
    source_list: ["migu", "qmusic", "netease"]
    """
    folder_list = TaskRecord.objects.filter(batch=batch, icon="icon-folder").all()
    # 使用集合去重，防止如果用户同时勾选了父目录和子目录导致重复添加任务
    processed_files = set()
    bulk_set = []
    
    for folder in folder_list:
        # 支持递归扫描无限深度目录
        for root, dirs, files in os.walk(folder.full_path):
            # 排除 attachments 文件夹
            ...
            for each in files:
                ...
                full_path = os.path.join(root_forward, each).replace("\\", "/").replace("//", "/")
                
                if full_path not in processed_files:
                    if skip_processed:
                        task_record = Task.objects.filter(full_path=full_path).first()
                        if task_record and task_record.state == "success":
                            print(f"[{each}] 已成功挂削，根据跳过选项已跳过")
                            continue
    ...
    if bulk_set:
        try:
            # 降低批量插入的数量限制，防止 sqlite3.OperationalError: too many terms in compound SELECT
            TaskRecord.objects.bulk_create(bulk_set, batch_size=100)
        except Exception as e:
            print(f"创建批量任务失败: {e}")
    ...
    for task in task_list:
        ...
        for resource in source_list:
            ...
            try:
                ...
                print(f"匹配 {resource} 时发生异常: {e}")
                is_match = False
                # 这里应该继续尝试下一个音源，而不是中断整个循环
                continue
